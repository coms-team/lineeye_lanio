...
std::unique_ptr<LanIODevice> LanIO::get_device(int id) {
  return std::make_unique<LanIODevice>(id);
}
std::unique_ptr<LanIODevice> LanIO::get_device(const std::string& ipaddress) {
  return std::make_unique<LanIODevice>(ipaddress);
}

LanIODevice::LanIODevice(int id) {
  handle_ = LELanioConnect(id);
  if (handle_ == -1) {
    throw LanIOException((boost::format("could not connect to %1%") % id).str());
  }
}

LanIODevice::LanIODevice(const std::string& ipaddress) {
  // handle_ = LELanioConnectByIpAddress(const_cast<char*>(ipaddress.c_str()));
  handle_ = LELanioConnectDirect(const_cast<char*>(ipaddress.c_str()), 100);
  if (handle_ == -1) {
